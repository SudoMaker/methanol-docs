...
description: Render a sidebar or menu from ctx.pagesTree.
weight: 3
starryNight: true
---
...
- Prefer `routeHref` for `<a href>` (it includes `site.base`).
- Directory index routes use a trailing slash in `routePath` (e.g. `/guide/`).

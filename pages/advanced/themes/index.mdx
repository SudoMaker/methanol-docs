...
- `ctx`: page/site context (`ctx.page`, `ctx.pages`, `ctx.pagesTree`, `ctx.pagesByRoute`, `ctx.languages`, `ctx.language`, `ctx.site`, `ctx.withBase`, `ctx.routePath`, `ctx.routeHref`, `ctx.path`, `ctx.getSiblings`)

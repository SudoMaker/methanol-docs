...
- `ctx.withBase(path)`: prefixes `site.base`/Vite base for URLs that start with `/` (leaves relative URLs unchanged)
...
- `routeHref`: base-aware href for links (includes `site.base` / Vite base)

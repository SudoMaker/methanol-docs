...
- `ctx.pages`: Flat list of pages for navigation (excludes `exclude: true` and omits `/404` + `/offline` unless `hidden: false`).
- `ctx.pagesByRoute`: `Map` of `routePath -> page` for fast lookup.
...
- `ctx.routePath` / `ctx.routeHref`: Route information for the current page.
- `ctx.path`: Absolute filesystem path for the current page.
...
- `name`, `owner`, `base`, `mode`
- `root`, `pagesDir`, `componentsDir`, `publicDir`, `distDir`
- `pagefind` (`enabled`, `options`, `build`)
- `feed` (`enabled`, `atom`, `path`, `href`)
- `pwa` (`enabled`, `manifestPath`, `manifestHref`)
- `generatedAt`
...
- `hiddenByParent`: String or `null`, the nearest hidden ancestor route (e.g., `/hidden/`).
- `hiddenByParents`: Boolean, `true` if the page is inside any hidden section.

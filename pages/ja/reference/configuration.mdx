...
Methanol の内蔵 PWA 機能（自前の Service Worker とプリキャッシュマニフェスト）を有効化します。デフォルト: `false`。
...
- `precache` は `precache-manifest.json` を制御し、内蔵 Service Worker がインストール/ウォームアップに利用します。
- PWA の挙動を完全にカスタマイズしたい場合は `pwa: false` に設定し、独自の manifest / Service Worker を用意してください。

...
静的アセットフォルダ（そのままコピー）。デフォルトは `public`。`false` にすると無効化します。
...
テーマ設定。使い方は [Advanced themes](/ja/advanced/themes/) を参照。

主なフィールド:

- `theme.root`: テーマのルートディレクトリ（必須）
- `theme.template`: HTML テンプレート関数（`{ ctx, Page, ExtraHead, HTMLRenderer, components }` を受け取ります）
- `theme.components`: デフォルトの MDX コンポーネント
- `theme.componentsDir`: テーマコンポーネントのフォルダ
- `theme.pagesDir`: テーマページのフォルダ
- `theme.publicDir`: テーマの public アセット

...
Enables Methanol's built-in PWA support (custom service worker + precache manifest). Default: `false`.
...
- `precache` controls `precache-manifest.json`, which the built-in service worker uses for install/warmup caching.
- To fully customize PWA behavior, set `pwa: false` and provide your own manifest/service worker.

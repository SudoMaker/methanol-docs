...
Controls syntax highlighting via Starry Night (rehype-starry-night). Default: `false`.
...
- The CLI flag `--highlight` can be used to override this setting per-execution.
...
Detailed theme configuration. Refer to the [Advanced Theme Guide](../advanced/themes/) for complete specifications.
...
- `--search` overrides `pagefind.enabled`.
- `--pwa` overrides the `pwa` toggle.

...
- `ctx.pages`: 包含全站页面（含隐藏页及索引页）的扁平化集合（排除 `exclude: true`，且除非显式设置 `hidden: false`，否则不包含 `/404` 和 `/offline`）。
- `ctx.pagesByRoute`: `routePath -> page` 的 `Map` 对象，用于快速查询。
...
- `ctx.routePath` / `ctx.routeHref`: 当前页面的路由信息。
- `ctx.path`: 当前页面的绝对文件系统路径。
...
- `hiddenByParent`: 字符串或 `null`，最近的隐藏父级路由（例如 `/hidden/`）。
- `hiddenByParents`: 布尔值，若页面位于任意隐藏分区下则为 `true`。

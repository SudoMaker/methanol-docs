...
- `ctx`: 页面与站点上下文（`ctx.page`, `ctx.pages`, `ctx.pagesTree`, `ctx.pagesByRoute`, `ctx.languages`, `ctx.language`, `ctx.site`, `ctx.withBase`, `ctx.routePath`, `ctx.routeHref`, `ctx.path`, `ctx.getSiblings`）
- `page`: `ctx.page` 别名，提供页面信息
- `withBase`: URL Base 处理函数（等同于 `ctx.withBase`）
- `HTMLRenderer`: refui HTML 渲染器（提供 `rawHTML` 与 `serialize` 方法，或用于手动渲染 JSX 组件）
- `components`: 合并后的组件集合（包括主题组件与用户自定义组件，适用于导航、搜索等布局级组件）

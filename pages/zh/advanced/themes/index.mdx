...
- [主题实现指南](../theme-guide/)
- [渲染导航](../theme-guide/navigation)
- [渲染目录（ToC）](../theme-guide/toc)
- [上一页/下一页](../theme-guide/prev-next)
- [语言切换](../theme-guide/language-switch)
- [专门用途页面](../theme-guide/special-pages)
...
主题的 `public/` 会在 dev/build 时合并到项目的 `publicDir`。若同一路径同时存在，则项目资源优先。

## 模板约定

`theme.template` 接收：

- `PageContent`: 当前页面内容的 JSX 组件
- `ExtraHead`: 注入到 `<head>` 的额外内容（页面资源 + Methanol 运行时 + 页面 `<Head>`/`head` 输出）
- `ctx`: 页面/站点上下文（`ctx.page`, `ctx.pages`, `ctx.pagesTree`, `ctx.pagesByRoute`, `ctx.languages`, `ctx.language`, `ctx.site`, `ctx.withBase`）
- `page`: `ctx.page` 的别名
- `withBase`: 用于生成带 base 的 URL（与 `ctx.withBase` 相同）
...
当站点部署在子路径下（`site.base !== '/'`）时：

- 对需要跟随 Vite `base` 的 URL（例如 Vite 管理的运行时资源）使用 `withBase('/...')`（或 `ctx.withBase('/...')`）。
- 一般不需要对 `theme.sources` 的 URL 或 `public/` 下的静态文件使用 `withBase()`（Methanol 会在构建时进行解析）。但如果你在 JS 中动态生成 URL 并遇到 Vite dev 下解析不一致，`withBase()` 仍然是一个实用的规避手段。

...
- `site.name`: 站点名称/标题
- `site.owner`: 站点拥有者/作者（用于博客主题页脚与订阅源）
...
配置站点主题。可以是主题对象（见下文）或字符串。当指定为字符串时，Methanol 按以下优先级进行解析：

1.  **内置主题：** 匹配 Methanol 内部预置的主题（例如 `default`、`blog`）。
2.  **包解析：** 搜索名为 `methanol-theme-xxx`（`xxx` 为你的输入内容）的 npm 包。
3.  **本地路径：** 解析为指向主题目录或入口文件的相对或绝对路径。

关于主题对象的完整规范，请参阅 [主题（进阶）](../advanced/themes/)。
...
- 设置 `atom: true` 输出 Atom（默认路径：`/rss.xml` 或 `/atom.xml`）。
- `siteUrl` 必须是绝对 URL。若未提供，将使用 `site.base`，因此请设置为完整域名（例如 `https://example.com/`）。
- 隐藏页面不会进入订阅源，`limit` 默认是 `10`。
- 订阅项优先使用 `frontmatter.author`，没有则使用 `site.owner`。

### `pwa`

启用 Methanol 内置的 PWA 支持（包含自定义 Service Worker 及预缓存清单）。默认值：`false`。
...
- `manifest` 配置将合并至生成的 `manifest.webmanifest` 中。
- `precache` 配置控制 `precache-manifest.json` 的生成，供内置 Service Worker 用于安装和预热缓存。
- `precache.priority`: 优先缓存的 glob 模式列表（将被移至清单前部）。
- `precache.limit`: 预缓存清单的最大条目数限制（`null` 表示无限制）。
- `precache.batchSize`: Service Worker 在安装及预热时的并发请求数量。
- 如需完全自定义 PWA 行为，请设置 `pwa: false` 并自行提供 Manifest 和 Service Worker。

### `starryNight`

代码高亮配置（基于 `rehype-starry-night`）。默认值：`true`。
...
- CLI 参数 `--highlight`/`--no-highlight` 可运行时切换是否启用全局代码高亮。
- Methanol 会自动加载 fenced code 中出现的语言对应语法。仅在语言歧义或 Starry Night 未支持的情况下才需要配置 `starryNight.grammars`。

### `jobs`

构建时的 worker 线程数。默认值：`0`（根据页面数量自动设置）。

```js
jobs: 0 // 自动（round(ln(页面数))，不超过 CPU 核心数）
// or
jobs: 4
```

- CLI 参数 `--jobs`（`-j`）可在单次执行时覆盖此设置。
...
- `--search`/`--no-search` 覆盖 `pagefind.enabled`
- `--rss`/`--no-rss` 覆盖 `feed.enabled`
- `--atom`/`--no-atom` 覆盖 `feed.atom`
- `--pwa`/`--no-pwa` 覆盖 `pwa` 开关
- `-v`/`--verbose` 开启详细日志模式
- `--config` 指定配置文件路径

...
启用 Methanol 内置的 PWA 支持（自带 Service Worker 与预缓存清单）。默认值：`false`。
...
- `manifest` 会合并到生成的 `manifest.webmanifest` 中。
- `precache` 控制 `precache-manifest.json`，内置 Service Worker 会根据它进行安装与预热缓存。
